...
function requestToPythonRequest(
  request: Request,
  warnings: Warnings = [],
  imports: Set<string>,
...
export function _toPythonRequest(
  requests: Request[],
  warnings: Warnings = []
): string {
  const code = [];
  ...
  for (const request of requests) {
    const requestCode = requestToPythonRequest(
      request,
      warnings,
      imports,
      thirdPartyImports
...
export function toPythonRequestWarn(
  curlCommand: string | string[],
  warnings: Warnings = []
): [string, Warnings] {
  const requests = parseCurlCommand(curlCommand, supportedArgs, warnings);
  const python = _toPythonRequest(requests, warnings);
  return [python, warnings];
}

export function toPythonRequest(curlCommand: string | string[]): string {
  return toPythonRequestWarn(curlCommand)[0];
}
